...
void ConsensusMatchingTracker::initialize(cv::Mat im_gray0, long topLeftx, long topLefty, long width,
                     long height) {
...
void ConsensusMatchingTracker::processFrame(cv::Mat &im_gray, cv::Mat &im_rgba) {
    ...
    char message[100];
    //sprintf(message, "center.x = %f", center.x);
    sprintf(message, "isInitialized() = %d", isInitialized());
    cv::putText(im_rgba, message, cv::Point(im_rgba.cols / 2 - 50, im_rgba.rows / 2), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,200,200), 2);
}
